...
def get_relative_path(absolute_path: str, codebase_path: str) -> str:
    ...
    try:
        ...
        if rel_path.startswith('..'):
            raise ValueError(f"The absolute path '{absolute_path}' is not within the codebase path '{codebase_path}'")
            
        return rel_path
    except Exception as e:
        raise ValueError(f"Error converting absolute path to relative path: {str(e)}")

def get_relative_paths(absolute_paths: List[str], codebase_path: str) -> List[str]:
    """
    ...
    """
    return [get_relative_path(abs_path, codebase_path) for abs_path in absolute_paths]
...
def get_absolute_path(relative_path: str, codebase_path: str) -> str:
    ...
    try:
        ...
        if not abs_path.startswith(base_path):
            raise ValueError(f"The resulting absolute path '{abs_path}' is not within the codebase path '{base_path}'")
            
        return abs_path
    except Exception as e:
        raise ValueError(f"Error converting relative path to absolute path: {str(e)}") 