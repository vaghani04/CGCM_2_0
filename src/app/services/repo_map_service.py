...
class RepositoryMapService:
    ...
    async def generate_repository_map(
        ...
        for file_path in files_to_analyze:
            try:
                ...
                loggers["main"].error(
                    f"Warning: Failed to analyze {file_path}: {e}"
                )
                continue

        # Generate directory structure
        directory_structure = self._generate_directory_structure(
            codebase_path, files_to_analyze
        )
        ...
        repo_map = RepositoryMap(
            ...
            metadata={
                ...
                "supported_languages": list(
                    set(f.language.value for f in analyzed_files)
                ),
            },
        )
    ...
    def _find_supported_files(self, root_path: Path) -> List[str]:
        ...
        for root, dirs, files in os.walk(root_path):
            ...
            dirs[:] = [
                d for d in dirs if d not in settings.REPO_MAP_EXCLUDED_DIRS
            ]
    ...
    async def _analyze_file(self, file_path: str) -> Optional[FileInfo]:
        ...
        language = self.supported_extensions.get(
            extension, LanguageType.UNKNOWN
        )
        ...
        try:
            ...
            if not file_path.exists():
                loggers["main"].error(
                    f"Warning: File does not exist: {file_path}"
                )
                return None
            ...
            if language == LanguageType.PYTHON:
                ...
                return await self._analyze_js_ts_file(
            ...
            loggers["main"].error(
                f"Warning: Permission denied: {file_path} - {e}"
            )
            return None
        except UnicodeDecodeError as e:
            loggers["main"].error(
                f"Warning: Unicode decode error in file {file_path}: {e}"
            )
            return None
        except Exception as e:
            loggers["main"].error(
                f"Warning: Error analyzing file {file_path}: {type(e).__name__}: {e}"
            )
            return None
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                ...
                class_info = ClassInfo(
                    name=node.name,
                    bases=[
                        self._extract_base_name(base) for base in node.bases
                    ],
                    methods=methods,
    ...
    async def _analyze_js_ts_file(
        self, file_path: str, content: str, language: LanguageType
    ) -> FileInfo:
            ...
            for pattern in patterns:
                ...
                for match in matches:
                    ...
                    func_info = FunctionInfo(
                        ...
                        docstring=self._extract_js_function_comment(
                            lines, i - 1
                        ),
                        line_number=i,
                        is_async=is_async,
    ...
    def _extract_js_classes(self, content: str) -> List[ClassInfo]:
        ...
        while i < len(lines):
            ...
            if class_match:
                ...
                base_class = (
                    class_match.group(2) if class_match.group(2) else None
                )
    ...
    def _generate_directory_structure(
        self, root_path: Path, files: List[str]
    ) -> Dict[str, any]:
    ...
    def _generate_summary_stats(self, files: List[FileInfo]) -> SummaryStats:
        ...
        for file_info in files:
            ...
            file_complexity.append(
                (file_info.path, complexity, file_info.lines_of_code)
            )
        ...
        if file_complexity:
            file_complexity_by_comp = sorted(
                file_complexity, key=lambda x: x[1], reverse=True
            )
            most_complex_files = [
                {"path": path, "complexity": comp}
                for path, comp, _ in file_complexity_by_comp[:10]
            ]

            file_complexity_by_size = sorted(
                file_complexity, key=lambda x: x[2], reverse=True
            )
            largest_files = [
                {"path": path, "lines": lines}
                for path, _, lines in file_complexity_by_size[:10]
            ]
    ...
    async def _save_repository_map(
        self, repo_map: RepositoryMap, output_file: str
    ):
        ...
        try:
            ...
            raise Exception(
                f"Failed to save repository map to {output_file}: {e}"
            )
    ...
    def _extract_js_function_comment(
