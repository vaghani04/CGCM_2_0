...
class MerkleTree:
    ...
    def __init__(self, leaves: List[bytes]):
        ...
        self.leaves = [
            leaf if isinstance(leaf, bytes) else bytes(leaf) for leaf in leaves
        ]
...
class MerkleTreeService:
    ...
    def build_merkle_tree(
    ...
    ) -> Tuple[MerkleTree, Dict[str, bytes]]:
        """
        ...
        for root, dirs, files in os.walk(codebase_path):
            ...
            # For non-root directories, apply ignore rules
                ...
                ignored in rel_path.split(os.path.sep)
                for ignored in ignore_dirs
            ...
            for file in files:
                ...
                if any(
                    file.lower().endswith(ext) for ext in exclude_extensions
                ):
                    continue

                # Only include files with specified extensions
                if not any(
                    file.lower().endswith(ext) for ext in include_extensions
                ):
                    continue
                ...
                if os.path.dirname(relative_path) and any(
                    ignored in relative_path.split(os.path.sep)
                    for ignored in ignore_dirs
                ):
                    continue
        ...
        if not hash_values:
            hash_values = [
                b"empty"
            ]  # Add a default value if there are no files
    ...
    def get_changed_files(self, codebase_path: str) -> FileChangeResult:
        ...
        changed_files, deleted_files = self.compare_merkle_trees(
            old_tree, new_tree, old_file_hashes, new_file_hashes
        )
    ...
    def compare_merkle_trees(
        self,
        old_tree: MerkleTree,
        new_tree: MerkleTree,
        old_file_hashes: Dict[str, bytes],
        new_file_hashes: Dict[str, bytes],
        ...
        for file_path, file_hash in new_file_hashes.items():
            if (
                file_path not in old_file_hashes
                or old_file_hashes[file_path] != file_hash
            ):
