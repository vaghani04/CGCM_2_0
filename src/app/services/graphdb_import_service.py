...
class GraphDBImportService:
    ...
    async def import_repository_map(
        self, repo_map_data: Dict
    ) -> Dict[str, Any]:
    ...
    async def _setup_database_schema(self):
        ...
        constraints = [
            ...
            GraphConstraintRequest(
                NodeType.FUNCTION, ["name", "file_path", "line_number"]
            ),
            GraphConstraintRequest(
                NodeType.CLASS, ["name", "file_path", "line_number"]
            ),
        ]
    ...
    async def _create_directory_structure(self, repo_map: Dict, stats: Dict):
        ...
        root_node = DirectoryNode(
            path=root_path, name=os.path.basename(root_path) or "root"
        )
    ...
    async def _create_directory_nodes_recursive(
        self, structure: Dict, parent_path: str, parent_id: str, stats: Dict
    ):
    ...
    async def _create_file_directory_relationships(
        self, files: List[Dict], stats: Dict
    ):
        ...
        results = await self.neo4j_service.batch_create_relationships(
            relationships
        )
    ...
    async def _create_code_entities(self, repo_map: Dict, stats: Dict):
        ...
        # Batch create nodes
        function_ids = await self.neo4j_service.batch_create_nodes(
            function_nodes
        )
    ...
    async def _create_code_entity_relationships(
        self,
        repo_map: Dict,
        function_nodes: List,
        function_ids: List,
        ...
        await self._create_class_method_relationships(
            repo_map, class_nodes, class_ids, relationships, stats
        )
    ...
    async def _create_class_method_relationships(
        self,
        repo_map: Dict,
        class_nodes: List,
        class_ids: List,
        ...
        if method_nodes:
            ...
            # Create class-method relationships
            for (method_node, class_id), method_id in zip(
                method_nodes, method_ids
            ):
                if method_id:
                    relationship = GraphRelationship(
    ...
    async def _create_import_relationships(self, repo_map: Dict, stats: Dict):
        ...
        if import_nodes:
            ...
            relationships = []
            for (import_node, file_id), import_id in zip(
                import_nodes, import_ids
            ):
                if import_id:
                    relationship = GraphRelationship(
    ...
    async def _create_dependency_relationships(
        self, repo_map: Dict, stats: Dict
    ):
