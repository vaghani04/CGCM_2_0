...
class Neo4jService:
    ...
    async def create_relationship(
        ...
        if relationship.properties:
            props_str = (
                "{"
                + ", ".join(
                    [f"{k}: ${k}" for k in relationship.properties.keys()]
                )
                + "}"
            )
    ...
    async def _create_nodes_individually(
    ...
    async def _create_relationship_batch(
        self, relationships: List[GraphRelationship]
    ) -> List[bool]:
        ...
        with self.driver.session() as session:
            ...
            try:
                ...
                return await self._create_relationships_individually(
                    relationships
                )

    async def _create_relationships_individually(
        self, relationships: List[GraphRelationship]
    ) -> List[bool]:
    ...
    async def clear_database(self) -> bool:
        ...
        query = GraphQuery(cypher_query="MATCH (n) DETACH DELETE n")
    ...
    async def create_indexes(
        self, indexes: List[GraphIndexRequest]
    ) -> List[bool]:
    ...
    async def create_constraints(
        self, constraints: List[GraphConstraintRequest]
    ) -> List[bool]:
            ...
            try:
                if constraint_req.constraint_type == "UNIQUE":
                            ...
                            for record in index_result.records:
                                ...
                                if index_name:
                                    try:
                                        ...
                                        loggers["main"].info(
                                            f"Dropped existing index: {index_name}"
                                        )
                                    except Exception as drop_err:
                                        loggers["main"].error(
                                            f"Error dropping index {index_name}: {drop_err}"
                                        )
                        except Exception as idx_err:
                            loggers["main"].error(
                                f"Error checking indexes: {idx_err}"
                            )

                    # Now create the constraint
                    properties = ", ".join(
                        [f"n.{prop}" for prop in constraint_req.property_names]
                    )
                    ...
                    try:
                        ...
                        loggers["main"].error(
                            f"Constraint creation failed, falling back to index: {create_err}"
                        )
                        for prop in constraint_req.property_names:
                            ...
                            try:
                                ...
                                loggers["main"].error(
                                    f"Failed to create fallback index: {idx_create_err}"
                                )
                        results.append(False)
                else:
                    loggers["main"].error(
                        f"Unsupported constraint type: {constraint_req.constraint_type}"
                    )
                    results.append(False)
            except Exception as e:
    ...
    async def find_nodes_by_property(
        self, node_type: NodeType, property_name: str, property_value: Any
    ) -> List[Dict[str, Any]]:
