...
class CodebaseInfoExtractionService:
    ...
    async def extract_codebase_info(
        ...
        try:
            directory_structure = await self.get_directory_structure(
                codebase_path, depth=4
            )
            code_patterns = await self._extract_code_patterns(codebase_path)
            previous_data = (
                await self.file_storage_service.get_from_file_storage(
                    f"{codebase_path}:{git_branch_name}",
                    file_name="code_patterns.json",
                )
            )

            if code_patterns and previous_data:
                is_similar, similarity_score = self.is_similar_enough(
                    code_patterns, previous_data, threshold=0.75
                )
                if is_similar:
                    return {
            ...
            documentation_content = await self._extract_documentation(
                codebase_path
            )
    ...
    async def get_directory_structure(
        self, codebase_path: str, depth: int = 2
    ) -> str:
        """
        ...
        with open(
            "intermediate_outputs/nl_context_gather_outputs/directory_structure.txt",
            "w",
            encoding="utf-8",
        ) as f:
    ...
    async def _extract_code_patterns(self, codebase_path: str) -> str:
            ...
            patterns.extend(
                self._grep_patterns(
                    ...
                    [
                        ...
                        # Database
                        (
                            r"SQLAlchemy|sessionmaker|Session",
                            "SQLAlchemy Usage",
                        ),
            ...
            patterns.extend(
                self._grep_patterns(
                    ...
                    [
                        ...
                        (
                            r"validate|validator|ValidationError",
                            "Validation Logic",
                        ),
        ...
        with open(
            "intermediate_outputs/nl_context_gather_outputs/code_patterns.txt",
            "w",
            encoding="utf-8",
        ) as f:
            f.write("\n".join(patterns))
    ...
    def _grep_patterns(
        self, codebase_path: str, pattern_list: List[tuple]
    ) -> List[str]:
        ...
        for pattern, description in pattern_list:
            try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    if match_count > 0:
                        results.append(
                            f"{description}: Found {match_count} matches"
                        )
    ...
    def _extract_documented_code(self, codebase_path: str) -> List[str]:
        ...
        try:
                ...
                if result.returncode == 0 and result.stdout.strip():
                    ...
                    if output_lines:
                        ...
                        for line in output_lines[:3]:  # Limit to 3 examples
                            if ":" in line:
                                try:
                                    file_path, line_num, content = line.split(
                                        ":", 2
                                    )
                                    ...
                                    results.append(
                                        f"  {rel_path}:{line_num} - {content.strip()}"
                                    )
                                except ValueError:
                                    continue
            ...
            if (
                docstring_result.returncode == 0
                and docstring_result.stdout.strip()
            ):
                        ...
                        if ":" in line:
                            try:
                                ...
                                if len(content.strip()) > 100:
                                    content_clean += "..."
                                results.append(
                                    f"  {rel_path}:{line_num} - {content_clean}"
                                )
                            except ValueError:
                                continue
    ...
    async def _extract_documentation(self, codebase_path: str) -> str:
        ...
        with open(
            "intermediate_outputs/nl_context_gather_outputs/documentation_content.txt",
            "w",
            encoding="utf-8",
        ) as f:
            f.write("\n".join(doc_content))
