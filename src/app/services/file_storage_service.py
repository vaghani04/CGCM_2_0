...
class FileStorageService:
    ...
    def _get_workspace_dir(self, workspace_path: str) -> Path:
        ...
        if (
            not workspace_name
        ):  # Handle cases like "/" or where path ends with "/"
            workspace_name = "root"
    ...
    def _serialize_data(self, data: Dict) -> Dict:
        ...
        serialized["merkle_tree"] = base64.b64encode(
            pickle.dumps(data["merkle_tree"])
        ).decode("utf-8")
        ...
        for file_path, file_hash in data["file_hashes"].items():
            serialized["file_hashes"][file_path] = base64.b64encode(
                file_hash
            ).decode("utf-8")
    ...
    def _deserialize_data(self, data: Dict) -> Dict:
        ...
        deserialized["merkle_tree"] = pickle.loads(
            base64.b64decode(data["merkle_tree"])
        )
        ...
        for file_path, file_hash_b64 in data["file_hashes"].items():
            deserialized["file_hashes"][file_path] = base64.b64decode(
                file_hash_b64
            )
    ...
    def store_merkle_tree(self, key: str, merkle_tree, file_hashes):
        ...
        try:
            ...
            tree_data = {"merkle_tree": merkle_tree, "file_hashes": file_hashes}
    ...
    def get_merkle_tree(self, key: str) -> Optional[Tuple[Any, Dict]]:
        ...
        try:
            ...
            return (
                deserialized_data["merkle_tree"],
                deserialized_data["file_hashes"],
            )
    ...
    async def store_in_file_storage(
        self, key: str, insights: Dict | str, file_name: str
    ):
        """
    ...
    async def get_from_file_storage(
        self, key: str, file_name: str
    ...
    async def store_variable_in_file_storage(
        self, data: Dict, file_name: str, workspace_path: str
    ):
        """
    ...
    async def get_variable_from_file_storage(
        self, file_name: str, workspace_path: str
    ) -> Optional[Dict]:
        """
