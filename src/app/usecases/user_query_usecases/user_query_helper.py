...
from src.app.usecases.context_gather_usecases.context_gather_helper import (
    ContextGatherHelper,
)
from src.app.usecases.user_query_usecases.grep_search_usecase import (
    GrepSearchUsecase,
)
from src.app.usecases.user_query_usecases.nl_search_usecase import (
    NLSearchUsecase,
)
from src.app.usecases.user_query_usecases.rag_retrieval_usecase import (
    RAGRetrievalUsecase,
)
...
from src.app.utils.hash_calculator import calculate_hash, calculate_special_hash
...
class UserQueryHelper:
    def __init__(
        ...
        rag_retrieval_usecase: RAGRetrievalUsecase = Depends(
            RAGRetrievalUsecase
        ),
        context_gather_helper: ContextGatherHelper = Depends(
            ContextGatherHelper
        ),
    ...
    async def context_from_nl_insights(
        self, user_query_data: Dict[str, Any]
    ) -> str:
        start_time = time.time()
        current_git_branch = (
            await self.context_gather_helper.get_current_branch_name(
                user_query_data["codebase_path"]
            )
        )
        ...
        loggers["time_tracker"].info(
            f"Processing time for NL context: {processing_time:.2f} seconds"
        )
        return nl_context
    ...
    async def context_from_rag(self, user_query_data: Dict[str, Any]) -> str:
        ...
        context = await self.rag_retrieval_usecase.rag_retrieval(
            query,
            index_name,
            target_directories,
            current_git_branch,
        ...
        processing_time = end_time - start_time
        loggers["time_tracker"].info(
            f"Processing time for RAG context: {processing_time:.2f} seconds"
        )
        return context

    async def context_from_repo_map(
        self, user_query_data: Dict[str, Any]
    ) -> str:
        """
        ...
        try:
            ...
            with open(
                "intermediate_outputs/repo_map_search_outputs/directory_structure.txt",
                "w",
            ) as f:
                f.write(directory_structure)

            cypher_queries = (
                await self.repo_map_usecase._generate_cypher_queries(
            ...
            loggers["main"].info(
                f"Executing {len(cypher_queries)} Cypher queries"
            )
            ...
            with open(
                "intermediate_outputs/repo_map_search_outputs/cypher_queries_execution_results.json",
                "w",
            ) as f:
                json.dump(results, f)

            formatted_results = await self.repo_map_usecase.format_results(
                results
            )
            with open(
                "intermediate_outputs/repo_map_search_outputs/formatted_results.json",
                "w",
            ) as f:
            ...
            processing_time = end_time - start_time
            loggers["time_tracker"].info(
                f"Processing time for repo map context: {processing_time:.2f} seconds"
            )
    ...
    async def context_from_grep_search(
        self, user_query_data: Dict[str, Any]
    ) -> str:
        start_time = time.time()
        ...
        processing_time = end_time - start_time
        loggers["time_tracker"].info(
            f"Processing time for grep search context: {processing_time:.2f} seconds"
        )
        return result
