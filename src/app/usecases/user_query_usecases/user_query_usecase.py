...
from src.app.usecases.user_query_usecases.user_query_helper import (
    UserQueryHelper,
)
...
class UserQueryUseCase:
    def __init__(
        self, user_query_helper: UserQueryHelper = Depends(UserQueryHelper)
    ):
    ...
    async def execute(self, user_query: Dict[str, Any]):
        ...
        try:
            repo_map_context, rag_context, nl_context, grep_search_context = (
            ...
            loggers["main"].info(
                f"✓ NL context: {len(nl_context) if isinstance(nl_context, list) else 'included'}"
            )
            ...
            if rag_empty and repo_map_empty:
                ...
                print(
                    f"✓ Grep search context included (RAG and repo_map empty): {len(grep_search_context) if isinstance(grep_search_context, list) else 'included'}"
                )
            else:
                print(
                    "Grep search context excluded (RAG or repo_map have data)"
                )
            ...
            print(
                f"🎯 Total parallel context gathering time: {processing_time:.2f} seconds"
            )
            ...
            processing_time = end_time - start_time
            print(
                f"⏱️ Processing time before error: {processing_time:.2f} seconds"
            )
            raise e
