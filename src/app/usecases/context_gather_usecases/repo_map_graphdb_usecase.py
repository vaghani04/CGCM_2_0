...
class RepoMapGraphDBUseCase:
    ...
    async def generate_and_store_repo_map(
        self, codebase_path: str, output_file: str = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if clear_result["status"] != "success":
                print(
                    f"⚠️ Warning: Failed to clear GraphDB: {clear_result.get('error_message', 'Unknown error')}"
                )
            else:
            ...
            repo_map_result = (
                await self.repo_map_service.generate_repository_map(
                    codebase_path=codebase_path, output_file=output_file
                )
            )
            ...
            import_stats = (
                await self.graphdb_import_service.import_repository_map(
                    repo_map_data
                )
            )
            ...
            result = {
                ...
                "summary": {
                    "total_files_processed": repo_map_result[
                        "total_files_analyzed"
                    ],
                    "processing_time_seconds": repo_map_result[
                    ...
                    "relationships_created": import_stats[
            ...
            print(
                "✅ Repository map generation and GraphDB import completed successfully!"
            )
            return result
    ...
    async def clear_graphdb(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": (
                    "GraphDB cleared successfully"
                    if success
                    else "Failed to clear GraphDB"
                ),
            }
        except Exception as e:
            return {"status": "error", "error_message": str(e)}
    ...
    async def check_graphdb_connection(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "message": (
                    "GraphDB connection is active"
                    if is_connected
                    else "GraphDB connection is not available"
                ),
            }
        except Exception as e:
            return {
